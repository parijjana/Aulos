...
class ThemerCompiler {
  static final Map<int, ThemeData> _cache = {};
  ...
  static ThemeData compile(ThemerModel model) {
    ...
    final themeData = _compileInternal(model);
    _cache[hash] = themeData;
    return themeData;
  }

  static ThemeData _compileInternal(ThemerModel model) {
    ...
    return ThemeData(
      ...
      chipTheme: ChipThemeData(
        backgroundColor:
            colors.secondary?.withValues(alpha: 0.12) ?? colors.surface,
        labelStyle: TextStyle(color: colors.onSurface),
        shape: RoundedRectangleBorder(
          borderRadius: BorderRadius.circular(roundness),
          side: BorderSide(
            color:
                colors.secondary?.withValues(alpha: 0.2) ?? Colors.transparent,
          ),
        ),
      ),
  ...
  static TextTheme? _compileTextTheme(
    ThemerTypography? typography,
    Color textColor,
  ) {
